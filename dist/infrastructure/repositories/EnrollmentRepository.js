...
let EnrollmentRepository = class EnrollmentRepository {
    count(args) {
        return db_1.default.enrollment.count(args);
    }
    ;
    findMany(args) {
        return db_1.default.enrollment.findMany(args);
    }
    ;
    findUnique(args) {
        return db_1.default.enrollment.findUnique(args);
    }
    ;
    findFirst(args) {
        return db_1.default.enrollment.findFirst(args);
    }
    ;
    update(args, transaction) {
        return (transaction || db_1.default).enrollment.update(args);
    }
    ;
    delete(id, transaction) {
        return (transaction || db_1.default).enrollment.delete({
            where: {
                id
            }
        });
