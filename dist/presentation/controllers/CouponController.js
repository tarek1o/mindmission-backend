...
let CouponController = class CouponController {
    constructor(couponService, logService) {
        this.couponService = couponService;
        this.logService = logService;
        this.getAllCoupons = (0, express_async_handler_1.default)(async (request, response, next) => {
            ...
            response.status(HTTPStatusCode_1.default.OK).json(ResponseFormatter_1.ResponseFormatter.formate(true, 'All Coupons are retrieved successfully', promiseResult[0], promiseResult[1], findOptions.skip, findOptions.take));
        });
        this.getCouponById = (0, express_async_handler_1.default)(async (request, response, next) => {
            ...
            if (!Coupon) {
                throw new APIError_1.default('This Coupon does not exist', HTTPStatusCode_1.default.BadRequest);
            }
            response.status(HTTPStatusCode_1.default.OK).json(ResponseFormatter_1.ResponseFormatter.formate(true, 'The Coupon is retrieved successfully', [Coupon]));
        });
        this.createCoupon = (0, express_async_handler_1.default)(async (request, response, next) => {
            var _a;
            const { select, include } = RequestManager_1.RequestManager.findOptionsWrapper(request);
            ...
            this.logService.log("ADD", "COUPON", Object.assign({ id: createdCoupon.id }, request.body.input), request.user);
            response.status(HTTPStatusCode_1.default.Created).json(ResponseFormatter_1.ResponseFormatter.formate(true, 'The Coupon is created successfully', [createdCoupon]));
        });
        this.updateCoupon = (0, express_async_handler_1.default)(async (request, response, next) => {
            const { select, include } = RequestManager_1.RequestManager.findOptionsWrapper(request);
            ...
            this.logService.log("UPDATE", "COUPON", Object.assign({ id: +request.params.id }, request.body.input), request.user);
...
exports.CouponController = CouponController = __decorate([
    ...
    __param(0, (0, inversify_1.inject)('ICouponService')),
